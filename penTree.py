...
def list_extractor(dep_seq, bag_of_words,pos_seq):
    ...
    for dep in dep_seq:
        ...
        if anchor_from and anchor_to and anchor_from[0] == anchor_to[0]:
            ...
            focus.append(pos_seq[pos1]+" "+wlzer.lemmatize(bag_of_words[anchor_to[0] - 1],pos1))
            list.append(pos_seq[pos1]+" "+wlzer.lemmatize(bag_of_words[anchor_from[1] - 1],pos2))
            list.append(pos_seq[pos1]+" "+wlzer.lemmatize(bag_of_words[anchor_to[1] - 1],pos1))
...
def buildQueuePenTree(tree, queue):
    ...
    if type(tree) == ParentedTree:
        ...
        if tree.label() in prep_phrases:
            ...
            string = recurse_terminal(tree, string, clos_brackets)
...
def traverse_terminal_for_word(tree,str_out,close_brackets) :
    ...
    if all_close:
        ...
        for child in tree :
            traverse_terminal_for_word(child, str_out, clos_brackets)
...
def traverse_for_words(tree,word1,word2):
    ...
    for t in tree :
        if word1 in t.leaves() and word2 in t.leaves() : #and t.label() not in acc_pos :
            ...
            correct_child = t
        # elif word1 in t and t.label() in poss):
...
def extract_annotations(annotations):
    ...
    return (bag_of_words, pos_seq, ner_seq, chunk_seq, dep_seq, srls, syntax_tree)
...
if __name__ == "__main__":
    ...
    (bag_of_words, pos_seq, ner_seq, chunk_seq, dep_seq, srls, syntax_tree) = extract_annotations(annotations)
